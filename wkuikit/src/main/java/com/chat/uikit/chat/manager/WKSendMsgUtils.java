        ...
        if (msg.type == WKContentType.WK_IMAGE || msg.type == WKContentType.WK_GIF || msg.type == WKContentType.WK_VOICE || msg.type == WKContentType.WK_LOCATION || msg.type == WKContentType.WK_FILE) {
            ...
            if (!TextUtils.isEmpty(contentMsgModel.url)) {
                ...
                if (!TextUtils.isEmpty(contentMsgModel.localPath)) {
                    //图片
                    OkGoUpload.getInstance().getChatUploadFileUrl(msg.channelID, msg.channelType, contentMsgModel.localPath, (url, filePath) -> {
                        if (!TextUtils.isEmpty(url)) {
//                            WKUpload.getInstance().upload(url, contentMsgModel.localPath, msg.clientSeq, new WKUpload.IUploadBack() {
//                                @Override
...
//                                }
//                            });
                            OkGoUpload.getInstance().uploadFile(url, contentMsgModel.localPath, msg.clientSeq, new OkGoUpload.IUploadListener() {
            ...
            if (!TextUtils.isEmpty(videoMsgModel.cover) && !TextUtils.isEmpty(videoMsgModel.url)) {
                ...
                if (TextUtils.isEmpty(videoMsgModel.cover)) {
                    //封面还未上传
                    OkGoUpload.getInstance().getChatUploadFileUrl(msg.channelID, msg.channelType, videoMsgModel.coverLocalPath, (url, filePath) -> {
                        if (!TextUtils.isEmpty(url)) {
                            OkGoUpload.getInstance().uploadFile(url, videoMsgModel.coverLocalPath, UUID.randomUUID().toString().replaceAll("-", ""), new OkGoUpload.IUploadListener() {
                                @Override
                                public void onError() {
                                    listener.onUploadResult(false, msg.baseContentMsgModel);
                                }
                                ...
                                public void onUploadSuccess(String url) {
                                    ...
                                    OkGoUpload.getInstance().getChatUploadFileUrl(msg.channelID, msg.channelType, videoMsgModel.localPath, (url1, filePath) -> {
                                        if (!TextUtils.isEmpty(url1)) {
                                            OkGoUpload.getInstance().uploadFile(url1, videoMsgModel.localPath, msg.clientSeq, new OkGoUpload.IUploadListener() {
                                                @Override
                                                public void onError() {
                                                    listener.onUploadResult(false, msg.baseContentMsgModel);
                                                }
